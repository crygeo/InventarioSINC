...
public class ServiceUsuario : ServiceBase<Usuario>, ICustomObjs
{
    ...
    {
        var request = await GetRequest<Usuario>(HttpMethod.Put, $"{BaseUrl}/change-password",
    ...
    {
        var request = await GetRequest<Usuario>(HttpMethod.Put, $"{BaseUrl}/asignar-rol",
            new { UserId = userId, RolId = rolId });
